...
/*
  ...
  Transcribed from the FA-3 torch api here: https://github.com/Dao-AILab/flash-attention/blob/main/hopper/flash_api.cpp
*/
Flash_fwd_params init_params_fprop(
  ...
  ffi::Result<ffi::Buffer<ffi::DataType::F32>> softmax_lse_d
) {
  // Default params
  void *cu_seqlens_q_d = nullptr;
  void *cu_seqlens_k_d = nullptr;
  void *seqused_k = nullptr;
  void *p_d = nullptr;
  ...
  int window_size_right = is_causal ? 0 : -1;
  ...
  const std::vector<int64_t> query_strides = GetRowMajorStrides(query.dimensions());
  const std::vector<int64_t> key_strides = GetRowMajorStrides(key.dimensions());
  const std::vector<int64_t> value_strides = GetRowMajorStrides(value.dimensions());
  const std::vector<int64_t> out_strides = GetRowMajorStrides(out->dimensions());
  ...
  // Softmax sum
  params.softmax_lse_ptr = softmax_lse_d->untyped_data();
  ...
  // Convert p from float to int so we don't have to conlovert the random uint to float to compare.
  ...
  if (window_size_left < 0 && window_size_right >= 0) { window_size_left = seqlen_kv; }
  if (window_size_left >= 0 && window_size_right < 0) { window_size_right = seqlen_kv; }
...
ffi::Error FlashAttentionHopperF16FwdImpl(
  ...
  if (query.dimensions().size() != 4 || key.dimensions().size() != 4 || value.dimensions().size() != 4) {
    return ffi::Error(ffi::ErrorCode::kInvalidArgument, "query/key/value must be 4-dim.");
  }

  if (tile_count_semaphore.dimensions().size() != 1 && tile_count_semaphore.dimensions()[0] != 1) {
    return ffi::Error(ffi::ErrorCode::kInvalidArgument, "tile_count_semaphore must be 1-dim and hold a single int32 value.");
  }

  Flash_fwd_params params = init_params_fprop(query, key, value, tile_count_semaphore, softmax_scale, is_causal, res, softmax_lse);
  ...
  if (head_dim == 128) {
    ...
    return ffi::Error(ffi::ErrorCode::kInvalidArgument, "Only a head_dim if 128 is supported right now.");
  }
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    FlashAttentionHopperF16Fwd, FlashAttentionHopperF16FwdImpl,
    ffi::Ffi::Bind()
